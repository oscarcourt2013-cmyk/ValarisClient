...
public final class DiscordIpcClient {
        ...
        for (int i = 0; i < 10; i++) {
            try {
                if (tryConnectWindows(i) || tryConnectUnix(i)) {
                    handshake();
                    connected.set(true);
                    StellarClient.LOGGER.info("Discord Rich Presence connected (pipe {})", i);
                    return true;
                }
            } catch (IOException e) {
                closeTransport();
    ...
    private boolean sendActivity(DiscordPresenceSnapshot snapshot, boolean includeButtons) {
        try {
            ...
            StellarClient.LOGGER.debug("Discord IPC activity failed (buttons={}): {}", includeButtons, e.getMessage());
            connected.set(false);
            closeTransport();
            return false;
        }
