...
public final class ServerApiService {
    ...
    public void onPayload(String json) {
        ...
        switch (type) {
            case ServerApiProtocol.T_SERVER_ACCEPTED -> {
                ...
                String msg = ServerApiProtocol.str(obj, "message", "Server accepted StellarClient");
                notifications.success("Prime", msg);
                maybeAnnounceFriendsOnPartner();
            }
            case ServerApiProtocol.T_SERVER_REJECTED -> {
                ...
                notifications.warning("Prime", "Server rejected handshake: " + reason);
            }
    ...
    /** Convenience for hooks before {@link StellarClient} is fully wired in tests. */
    public static boolean tryHandleClientCommand(String message) {
        try {
            return StellarClient.get().serverApi().handleClientCommand(message);
