...
/**
 * In-game social hub â€” connects to the unified Prime backend on world join.
...
public final class SocialService {
    ...
    public SocialService(MinecraftAdapter adapter, NotificationManager notifications) {
        ...
        client.onEvent(event -> {
            ...
            if ("friend_request".equals(t)) {
                ...
                if (!text.isBlank()) {
                    notifications.info(from, text.length() > 48 ? text.substring(0, 48) + "â€¦" : text);
                } else if (m.has("imageUrl") && !m.get("imageUrl").isJsonNull()) {
                    notifications.info(from, "[image]");
                }
            } else if ("party_invite".equals(t)) {
                String name = event.has("fromUsername") ? event.get("fromUsername").getAsString() : "Player";
                notifications.info("Party", name + " invited you â€” open Social Hub to accept");
                ...
                notifications.info("Party",
                        "Party wants you to join " + addr + " â€” open Social Hub or press Join");
    ...
    public void onWorldJoin() {
        ...
        if (server != null && !server.isBlank() && !"Singleplayer".equalsIgnoreCase(server)) {
            ...
            // Only auto-share when already in a party â€” launcher can also share via setPartyServer.
    ...
    public void tick() {
        ...
        // Exponential backoff: 5s â†’ 10s â†’ 20s â†’ 30s cap.
