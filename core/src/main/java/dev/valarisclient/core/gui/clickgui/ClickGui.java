...
import dev.valarisclient.core.gui.menu.MainMenuRenderer;
import dev.valarisclient.core.gui.menu.OnboardingManager;
import dev.valarisclient.core.gui.menu.OnboardingScreen;
import dev.valarisclient.core.gui.component.ColorPickerWidget;
import dev.valarisclient.core.gui.component.TextInputField;
...
import dev.valarisclient.core.gui.clickgui.ModuleCardBrowser;
import dev.valarisclient.core.gui.menu.ConfigurationsMenuRenderer;
import dev.valarisclient.core.gui.menu.CosmeticsMenuRenderer;
import dev.valarisclient.core.gui.menu.SettingsMenuRenderer;
...
/**
 * Valaris ClickGUI with main menu, favorites, category panels and search.
 */
public final class ClickGui implements ConfigBinding {
    ...
    public ClickGui(ModuleManager modules, ThemeManager themes,
        ...
        favoritesPanel = new Panel(ValarisLang.get("valaris.gui.clickgui.favorites", "Favorites"),
        ...
        switch (view) {
            case MAIN_MENU -> {
                ...
                mainMenu.renderPanel(ctx, theme, px, py, adapter.playerName(),
                        ValarisDesign.VERSION, mouseX, mouseY, menuSlide);
            }
    ...
    private void renderEditors(RenderContext ctx, Theme theme) {
        if (stringEditor != null) {
            int[] pos = GuiLayout.clampPopup(editorX, editorY, 140, ValarisDesign.INPUT_HEIGHT,
    ...
    private void renderSearchBar(RenderContext ctx, Theme theme) {
        String label = searchQuery.isEmpty()
                ? ValarisLang.get("valaris.gui.clickgui.search.placeholder", "Type to search...")
        ...
        ctx.fillRoundedRect(x, y, width, SEARCH_HEIGHT, ValarisDesign.RADIUS_MD, theme.backgroundLight());
        ctx.fillGradientHorizontal(x + 2, y + SEARCH_HEIGHT - 2, width - 4, 2,
                theme.accent(), dev.valarisclient.core.util.ColorUtil.withAlpha(theme.accent(), 0.05f));
    ...
    private void openStringEditor(StringSetting setting, int x, int y) {
        ...
        stringEditor.setFocused(true);
        int[] pos = GuiLayout.clampPopup(x, y, 140, ValarisDesign.INPUT_HEIGHT, screenWidth, screenHeight);
        editorX = pos[0];
        editorY = pos[1];
        colorPicker = null;
        editingColor = null;
    ...
    private void rebuildSearchPanel() {
        ...
        searchPanel = new Panel(ValarisLang.get("valaris.gui.clickgui.search.panel", "Search (%d)", results.size()),
                results, favorites,
