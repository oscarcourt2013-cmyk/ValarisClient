...
final class ClickGuiTopBar {
    ...
    private static Solved solve(RenderContext ctx, ClickGuiBrowseLayout layout) {
        ...
        int logoW = ValarisLogo.widthForHeight(barH - 6);
        int wordmarkW = GuiLayout.labelWidth(ctx, "VALARIS CLIENT");
        ...
        for (int attempt = 0; attempt < 5; attempt++) {
            ...
            int rightW = (searchW > 0 ? searchW + ValarisDesign.SPACE_SM : 0) + CLOSE_BTN + EDGE_PAD;
            ...
            if (leftW + tabsW + rightW <= barW || last) {
                ...
                int searchX = closeX - ValarisDesign.SPACE_SM - searchW;
    ...
    void render(RenderContext ctx, Theme theme, ClickGuiBrowseLayout layout, ClickGuiView activeView,
        ...
        int logoH = barH - 6;
        ValarisLogo.draw(ctx, barX + EDGE_PAD, barY + (barH - logoH) / 2, logoH, 0xFFFFFFFF);
        if (s.showWordmark()) {
            GuiLayout.label(ctx, "VALARIS CLIENT",
                    barX + EDGE_PAD + ValarisLogo.widthForHeight(logoH) + 4,
        ...
        ctx.fillRoundedRect(s.closeX(), closeY, CLOSE_BTN, CLOSE_BTN, ValarisDesign.RADIUS_SM,
