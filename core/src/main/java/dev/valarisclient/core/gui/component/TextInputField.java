...
public final class TextInputField {
    ...
    public void render(RenderContext ctx, Theme theme, int x, int y, int width) {
        int h = ValarisDesign.INPUT_HEIGHT;
        ctx.fillRoundedRect(x, y, width, h, ValarisDesign.RADIUS_SM,
        ...
        ctx.fillGradientHorizontal(x + 1, y + h - 2, width - 2, 2,
                ...
                dev.valarisclient.core.util.ColorUtil.withAlpha(theme.accent(), 0.05f));
        ...
        int textX = x + ValarisDesign.SPACE_SM;
        int maxTextW = width - ValarisDesign.SPACE_SM * 2;
    ...
    public boolean hit(double mx, double my, int x, int y, int width) {
        return mx >= x && mx < x + width && my >= y && my < y + ValarisDesign.INPUT_HEIGHT;
    }
