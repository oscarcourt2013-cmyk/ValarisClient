...
import dev.valarisclient.core.cloud.CloudClient;
...
public final class ConfigurationsMenuRenderer {
    ...
    public void render(RenderContext ctx, Theme theme, CloudSyncManager cloud, ProfileManager profiles,
        ...
        ctx.drawText(ValarisLang.get("valaris.gui.configurations.title", "Configurations"),
                x + 12, y + 12, theme.accent(), true);
        ctx.drawText(ValarisLang.get("valaris.gui.configurations.local_only", "Local backups only — stored on this PC"),
                x + 12, y + 28, theme.foregroundMuted(), true);
        ctx.drawText(ValarisLang.get("valaris.gui.configurations.profile", "Profile: %s", profiles.activeProfile()),
                x + 12, y + 44, theme.foreground(), true);

        int btnY = y + 64;
        int btnW = (PANEL_W - 24 - BTN_GAP * 2) / 3;
        drawButton(ctx, theme, x + 12, btnY, btnW, BTN_H,
                ValarisLang.get("valaris.gui.configurations.upload", "Upload"), mouseX, mouseY, true);
        drawButton(ctx, theme, x + 12 + btnW + BTN_GAP, btnY, btnW, BTN_H,
                ValarisLang.get("valaris.gui.configurations.download", "Download"), mouseX, mouseY, false);
        drawButton(ctx, theme, x + 12 + (btnW + BTN_GAP) * 2, btnY, btnW, BTN_H,
                ValarisLang.get("valaris.gui.configurations.restore", "Restore"), mouseX, mouseY, false);
        ...
        ctx.drawText(ValarisLang.get("valaris.gui.configurations.versions", "Versions (%d):", versions.size()),
        ...
        if (versions.isEmpty()) {
            ctx.drawText(ValarisLang.get("valaris.gui.configurations.empty", "No backups yet — tap Upload"),
        ...
        ctx.drawText(ValarisLang.get("valaris.gui.configurations.hint", "Click a version, then Restore"),
    ...
    public boolean mousePressed(double mx, double my, int button,
        ...
        if (hit(mx, my, x + 12, btnY, btnW, BTN_H)) {
            ...
            statusMessage = ValarisLang.get("valaris.gui.configurations.status.uploaded", "Backup saved locally");
            return true;
        }
        if (hit(mx, my, x + 12 + btnW + BTN_GAP, btnY, btnW, BTN_H)) {
            if (cloud.downloadNow(profiles.activeProfile())) {
                profiles.saveActive();
                statusMessage = ValarisLang.get("valaris.gui.configurations.status.downloaded", "Backup restored");
            } else {
                statusMessage = ValarisLang.get("valaris.gui.configurations.status.none", "No backup found");
            }
            return true;
        }
        if (hit(mx, my, x + 12 + (btnW + BTN_GAP) * 2, btnY, btnW, BTN_H)) {
            ...
            if (selectedVersion >= 0 && selectedVersion < versions.size()) {
                ...
                if (cloud.restoreVersion(profiles.activeProfile(), id)) {
                    profiles.saveActive();
                    statusMessage = ValarisLang.get("valaris.gui.configurations.status.restored", "Version restored");
                } else {
                    statusMessage = ValarisLang.get("valaris.gui.configurations.status.failed", "Restore failed");
                }
            } else {
                statusMessage = ValarisLang.get("valaris.gui.configurations.status.pick", "Select a version first");
            }
            return true;
        }
