...
import dev.valarisclient.core.cosmetics.CosmeticItem;
...
public final class CosmeticsMenuRenderer {
    ...
    public void render(RenderContext ctx, Theme theme, CosmeticManager cosmetics,
        ...
        GuiLayout.label(ctx, ValarisLang.get("valaris.gui.cosmetics.title", "Cosmetics"), x + 12, y + 10, theme.accent());

        int tabX = x + 8;
        for (CosmeticType type : SLOTS) {
            int tw = GuiLayout.tabWidth(ctx, ValarisLang.enumValue(type), 10);
            boolean sel = type == slot;
            ctx.fillRoundedRect(tabX, y + 26, tw, 14, ValarisDesign.RADIUS_SM,
                    sel ? theme.surfaceElevated() : theme.backgroundLight());
            GuiLayout.label(ctx, ValarisLang.enumValue(type), tabX + 4, y + 29, sel ? theme.accent() : theme.foregroundMuted());
            tabX += tw + 4;
        }
        ...
        String previewLabel = equipped != null
                ...
                : ValarisLang.get("valaris.gui.cosmetics.none", "None");
        ...
        for (CosmeticItem item : cosmetics.catalog().values()) {
            ...
            GuiLayout.label(ctx, ValarisLang.enumValue(item.rarity()), x + listW - 70, rowY + 5, theme.foregroundMuted());
            if (isEquipped) {
                GuiLayout.label(ctx, ValarisLang.get("valaris.gui.cosmetics.equipped", "Equipped"),
        ...
        GuiLayout.label(ctx,
                ValarisLang.get("valaris.gui.cosmetics.footer", "Visible to you + Valaris peers · Click to equip"),
