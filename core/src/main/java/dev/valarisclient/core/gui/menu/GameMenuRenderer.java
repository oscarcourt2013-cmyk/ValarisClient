...
/**
 * Valaris pause menu — dark desaturated world, red/black chrome, centered GAME MENU panel.
 */
public final class GameMenuRenderer {
    ...
    private void renderBranding(RenderContext ctx, Theme theme, GameMenuLayout layout, float fade) {
        ...
        ValarisLogo.drawCentered(ctx, centerX, layout.logoY(), layout.logoH(), 0xFFFFFFFF);

        float valarisScale = 1.05f;
        String valaris = "VALARIS";
        int valarisW = ctx.smoothTextWidth(valaris, valarisScale);
        ctx.drawSmoothText(valaris, centerX - valarisW / 2, layout.brandY(), theme.foreground(), valarisScale);
    ...
    private void renderPanel(RenderContext ctx, Theme theme, GameMenuLayout layout, float fade) {
        ...
        int w = layout.panelW();
        int h = layout.panelH();
        int radius = ValarisDesign.RADIUS_MD;
    ...
    private void renderPrimary(RenderContext ctx, Theme theme, MinecraftAdapter adapter,
        ...
        ctx.fillRoundedRect(x, y, w, h, ValarisDesign.RADIUS_SM, fill);
        ...
        if (hover) {
            ctx.fillRoundedBorder(x, y, w, h, ValarisDesign.RADIUS_SM, 1,
    ...
    private void renderFooter(RenderContext ctx, Theme theme, GameMenuLayout layout, float fade) {
        ...
        ValarisLogo.draw(ctx, 8, y + (h - 16) / 2, 16, 0xFFFFFFFF);
        ctx.drawSmoothText("ValarisClient", 28, textTop(ctx, y, h, brandScale),
        ...
        int ctaY = y + 4;
        int ctaH = h - 8;
        ctx.fillRoundedBorder(ctaX, ctaY, ctaW, ctaH, ValarisDesign.RADIUS_SM, 1,
