...
public final class LoadingOverlay {
    ...
    public void render(RenderContext ctx, Theme theme) {
        ...
        if (phase == Phase.HOLD || phase == Phase.ACTIVE) {
            ...
            int glowX = cx - ValarisLogo.widthForHeight(glowSize) / 2 - 12;
            int glowY = cy - 8;
            ctx.fillRoundedRect(glowX, glowY,
                    ValarisLogo.widthForHeight(glowSize) + 24, glowSize + 16,
        ...
        ValarisLogo.draw(ctx, -ValarisLogo.widthForHeight(baseLogoH) / 2, 0, baseLogoH, 0xFFFFFFFF);
        ctx.popTransform();
        ...
        String version = ValarisLang.get("valaris.gui.loading.version", "v%s", ValarisDesign.VERSION);
        String tagline = ValarisLang.get("valaris.gui.loading.tagline", "Premium Minecraft Client");
        ctx.drawText(version, barX, barY + ValarisDesign.SPACE_MD + 2, theme.foregroundMuted(), true);
        ctx.drawText(tagline, barX, barY + ValarisDesign.SPACE_MD + 14, theme.foregroundMuted(), true);
