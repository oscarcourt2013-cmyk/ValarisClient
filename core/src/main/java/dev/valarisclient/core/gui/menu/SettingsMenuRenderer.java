...
import dev.valarisclient.core.keybind.KeyNames;
...
public final class SettingsMenuRenderer {
    ...
    public enum Category {
        ...
        Category(String label) {
            this.label = label;
            this.key = "valaris.gui.settings.tab." + name().toLowerCase();
        }

        String translated() {
            return ValarisLang.get(key, label);
        }
    }
    ...
    public void render(RenderContext ctx, Theme theme, ThemeManager themes, ProfileManager profiles,
        ...
        GuiLayout.label(ctx, ValarisLang.get("valaris.gui.settings.title", "Settings"), x + 12, y + 10, theme.accent());
        ...
        for (Category cat : Category.values()) {
            ...
            ctx.fillRoundedRect(tabX, tabY, tw, 14, ValarisDesign.RADIUS_SM,
        ...
        switch (active) {
            case GENERAL -> {
                row(ctx, theme, x + 12, rowY,
                        ValarisLang.get("valaris.gui.settings.row.profile", "Profile"), profiles.activeProfile());
                rowY += 16;
                row(ctx, theme, x + 12, rowY,
                        ValarisLang.get("valaris.gui.settings.row.minecraft", "Minecraft"), adapter.minecraftVersion());
                rowY += 16;
                String sync = cloud.autoSync()
                        ? ValarisLang.get("valaris.gui.settings.cloud_sync.enabled", "Enabled")
                        : ValarisLang.get("valaris.gui.settings.cloud_sync.disabled", "Disabled");
                row(ctx, theme, x + 12, rowY,
                        ValarisLang.get("valaris.gui.settings.row.cloud_sync", "Local backup"), sync);
            }
            case APPEARANCE -> {
                row(ctx, theme, x + 12, rowY,
                        ValarisLang.get("valaris.gui.settings.row.active_theme", "Active theme"), themes.active().name());
                rowY += 18;
                drawThemeChip(ctx, theme, themes, "valaris-crimson",
                        ValarisLang.get("valaris.gui.settings.theme.crimson", "Crimson"),
                        x + 12, rowY, 100);
                drawThemeChip(ctx, theme, themes, "valaris-midnight",
                        ValarisLang.get("valaris.gui.settings.theme.midnight", "Midnight"),
                        x + 118, rowY, 100);
                drawThemeChip(ctx, theme, themes, "valaris-aurora",
                        ValarisLang.get("valaris.gui.settings.theme.aurora", "Aurora"),
                        x + 224, rowY, 100);
                rowY += 20;
                drawThemeChip(ctx, theme, themes, "valaris-obsidian",
                        ValarisLang.get("valaris.gui.settings.theme.obsidian", "Obsidian"),
                        x + 12, rowY, 100);
                drawThemeChip(ctx, theme, themes, "valaris-ember",
                        ValarisLang.get("valaris.gui.settings.theme.ember", "Ember"),
                        x + 118, rowY, 100);
            }
            case PERFORMANCE -> row(ctx, theme, x + 12, rowY,
                    ValarisLang.get("valaris.gui.settings.row.tip", "Tip"),
                    ValarisLang.get("valaris.gui.settings.tip.performance", "Use Performance Profiles module"));
            case CONTROLS -> renderControls(ctx, theme, keybinds, x, rowY, panelW, contentBottom);
            case ACCOUNT -> {
                row(ctx, theme, x + 12, rowY,
                        ValarisLang.get("valaris.gui.settings.row.player", "Player"), adapter.playerName());
                rowY += 16;
                String type = adapter.sessionAccountType();
                row(ctx, theme, x + 12, rowY,
                        ValarisLang.get("valaris.gui.settings.row.session", "Session"),
                ...
                String btn = ValarisLang.get("valaris.gui.settings.account.switch", "Switch account…");
                ...
                GuiLayout.label(ctx, GuiLayout.trimToWidth(ctx,
                                canSwitch
                                        ? ValarisLang.get("valaris.gui.settings.account.switch_hint",
                                        "Uses accounts from the ValarisClient Launcher")
                                        : ValarisLang.get("valaris.gui.settings.account.in_world",
                                        "Return to the title screen to switch accounts"),
                                panelW - 40),
                        x + 12, rowY, theme.foregroundMuted());
            }
            case PRIVACY -> row(ctx, theme, x + 12, rowY,
                    ValarisLang.get("valaris.gui.settings.row.data", "Data"),
                    ValarisLang.get("valaris.gui.settings.privacy.hint", "Configs stored locally only"));
            case ABOUT -> {
                ValarisLogo.draw(ctx, x + 12, rowY, 14, 0xFFFFFFFF);
                row(ctx, theme, x + 12 + ValarisLogo.widthForHeight(14) + 6, rowY + 2,
                        ValarisLang.get("valaris.gui.settings.about.client", "ValarisClient"),
                        ValarisLang.get("valaris.gui.settings.about.version", "v%s", ValarisDesign.VERSION));
                rowY += 20;
                row(ctx, theme, x + 12, rowY,
                        ValarisLang.get("valaris.gui.settings.about.legitimate", "Legitimate client"),
                        ValarisLang.get("valaris.gui.settings.about.tagline", "Visual & QoL only"));
            }
        }
        ctx.popClip();

        String footer = active == Category.CONTROLS
                ? ValarisLang.get("valaris.gui.settings.keybinds.footer",
                "Click key to rebind · Backspace clears · Scroll for more")
                : (search.isEmpty()
                ? ValarisLang.get("valaris.gui.settings.search.placeholder", "Search settings...")
                : search.toString());
    ...
    private void renderControls(RenderContext ctx, Theme theme, KeybindManager keybinds,
        ...
        for (Keybind bind : binds) {
            ...
            ctx.fillRoundedRect(btnX, drawY, KEY_BTN_W, 14, ValarisDesign.RADIUS_SM,
                    listening ? theme.accent() : theme.backgroundLight());
            String label = listening
                    ? ValarisLang.get("valaris.gui.settings.keybinds.listening", "Press a key…")
    ...
    public boolean mousePressed(RenderContext ctx, double mx, double my, int screenW, int screenH,
        ...
        if (active == Category.APPEARANCE) {
            ...
            if (mx >= x + 12 && mx < x + 112 && my >= rowY && my < rowY + 16) {
                themes.setActive("valaris-crimson");
                return true;
            }
            if (mx >= x + 118 && mx < x + 218 && my >= rowY && my < rowY + 16) {
                themes.setActive("valaris-midnight");
                return true;
            }
            if (mx >= x + 224 && mx < x + 324 && my >= rowY && my < rowY + 16) {
                themes.setActive("valaris-aurora");
                return true;
            }
            int row2 = rowY + 20;
            if (mx >= x + 12 && mx < x + 112 && my >= row2 && my < row2 + 16) {
                themes.setActive("valaris-obsidian");
                return true;
            }
            if (mx >= x + 118 && mx < x + 218 && my >= row2 && my < row2 + 16) {
                themes.setActive("valaris-ember");
                return true;
            }
        }
    ...
    private static void drawThemeChip(RenderContext ctx, Theme theme, ThemeManager themes,
        ...
        ctx.fillRoundedRect(x, y, w, 16, ValarisDesign.RADIUS_SM,
