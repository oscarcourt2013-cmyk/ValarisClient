...
/**
 * Draws and drives a Valaris-styled options screen from an {@link OptionsPage}.
 *
...
public final class OptionsMenuRenderer {
    ...
    private void renderHeader(RenderContext ctx, Theme theme, OptionsMenuLayout l, OptionsPage page) {
        ...
        if (l.showBranding()) {
            int logoH = 14;
            ValarisLogo.drawCentered(ctx, centerX, y, logoH, 0xFFFFFFFF);
            y += logoH + 3;
        }
    ...
    private void renderRow(RenderContext ctx, Theme theme, OptionRow row, int x, int y, int w, int h,
        ...
        switch (row) {
            case OptionRow.Toggle t -> {
                ...
                int tw = ValarisDesign.TOGGLE_WIDTH;
                int th = ValarisDesign.TOGGLE_HEIGHT;
                int tx = x + w - 6 - tw;
                int ty = y + (h - th) / 2;
