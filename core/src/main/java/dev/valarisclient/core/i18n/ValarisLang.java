package dev.valarisclient.core.i18n;

import dev.valarisclient.core.module.ModuleCategory;

import java.util.Locale;

/** Resolves ValarisClient UI strings via the bound Minecraft language manager. */
public final class ValarisLang {
    ...
    public static String module(String moduleId, String field, String fallback) {
        return get("valaris.module." + idKey(moduleId) + "." + field, fallback);
    }

    public static String setting(String moduleId, String settingId, String field, String fallback) {
        return get("valaris.module." + idKey(moduleId) + ".setting." + idKey(settingId) + "." + field, fallback);
    }

    public static String category(ModuleCategory category, String fallback) {
        return get("valaris.category." + category.name().toLowerCase(Locale.ROOT), fallback);
    }

    public static String hud(String elementId, String fallback) {
        return get("valaris.hud." + idKey(elementId), fallback);
    }
    ...
    public static String enumValue(Enum<?> value) {
        ...
        return get("valaris.enum." + enumKey + "." + valueKey, humanizeEnum(value.name()));
    }

    public static String cosmetic(String cosmeticId, String fallback) {
        return get("valaris.cosmetic." + idKey(cosmeticId), fallback);
    }
