...
public final class ValarisRewardHandler {

    private final NotificationManager notifications;
    private final MinecraftAdapter adapter;

    public ValarisRewardHandler(NotificationManager notifications, MinecraftAdapter adapter) {
    ...
    public void handle(JsonObject payload) {
        ...
        String title = ServerApiProtocol.str(payload, "title", "Valaris Reward");
        ...
        if ("link".equalsIgnoreCase(action) && !link.isBlank()) {
            ...
            if (!message.isBlank()) {
                adapter.displayClientMessage("§6[Valaris] §f" + message);
            }
        }
