package dev.valerisclient.core.discord.ipc;

import com.google.gson.JsonArray;
import com.google.gson.JsonObject;
import dev.valerisclient.core.ValerisClient;
import dev.valerisclient.core.discord.DiscordPresenceSnapshot;
...
public final class DiscordIpcClient {
    ...
    private final ExecutorService executor = Executors.newSingleThreadExecutor(r -> {
        Thread t = new Thread(r, "Valeris-Discord-RPC");
        t.setDaemon(true);
        return t;
    });
        ...
        for (int i = 0; i < 10; i++) {
            try {
                if (tryConnectWindows(i) || tryConnectUnix(i)) {
                    handshake();
                    connected.set(true);
                    ValerisClient.LOGGER.info("Discord Rich Presence connected (pipe {})", i);
                    return true;
                }
            } catch (IOException e) {
                closeTransport();
    ...
    private boolean sendActivity(DiscordPresenceSnapshot snapshot, boolean includeButtons) {
        try {
            ...
            ValerisClient.LOGGER.debug("Discord IPC activity failed (buttons={}): {}", includeButtons, e.getMessage());
            connected.set(false);
            closeTransport();
            return false;
        }
