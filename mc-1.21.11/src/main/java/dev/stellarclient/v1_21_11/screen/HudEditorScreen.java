...
import dev.stellarclient.v1_21_11.hud.VanillaHudLayerRenderer;
...
public final class HudEditorScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
        StellarClient client = StellarClient.get();
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        if (event.button() == 0 && StellarClient.get().hudEditor().mousePressed(event.x(), event.y())) {
            return true;
        }
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        if (event.button() == 0) {
            StellarClient.get().hudEditor().mouseDragged(event.x(), event.y(), width, height);
            return true;
        }
    ...
    public boolean mouseReleased(MouseButtonEvent event) {
        StellarClient.get().hudEditor().mouseReleased();
        return super.mouseReleased(event);
    }

    @Override
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        if (StellarClient.get().hudEditor().mouseScrolled(
    ...
    public boolean keyPressed(KeyEvent event) {
        if (StellarClient.get().hudEditor().keyPressed(event.key(), event.hasShiftDown(), event.hasControlDown())) {
            return true;
        }
    ...
    public void onClose() {
        ...
        StellarClient.get().hudEditor().flushAutosave();
