package dev.valarisclient.v1_21_11.network;
...
/** Registers Valaris presence + custom skin texture payloads for LAN / integrated servers. */
    ...
    public static void register() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(PresencePayload.TYPE, (payload, context) ->
                context.client().execute(() -> {
                    ValarisHooks.onPresencePayload(
        ...
        ClientPlayNetworking.registerGlobalReceiver(SkinTexturePayload.TYPE, (payload, context) ->
                context.client().execute(() -> {
                    if (payload.png() != null && payload.png().length <= CustomSkinService.MAX_BYTES) {
                        ValarisHooks.onSkinTexturePayload(payload.playerId(), payload.png());
                    }
                }));

        ValarisClient.get().presence().setNetworkAnnouncer(PresenceNetworking::sendLocalPresence);
