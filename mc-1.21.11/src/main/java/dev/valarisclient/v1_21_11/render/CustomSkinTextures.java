...
/**
 * Registers DynamicTextures for Valaris custom skins and builds overridden PlayerSkin values.
 *
    ...
    private static Identifier ensureRegistered(UUID uuid, String hash, byte[] png) {
        ...
        try {
            ...
            Identifier id = Identifier.fromNamespaceAndPath(
                    ValarisClient.MOD_ID, "skins/dynamic/" + uuid.toString().replace("-", ""));
            DynamicTexture texture = new DynamicTexture(
                    () -> "valaris-skin-" + hash.substring(0, Math.min(8, hash.length())), image);
            ...
            FAILED_HASHES.add(hash);
            ValarisClient.LOGGER.warn("Failed to upload custom skin for {}: {}", uuid, e.toString());
            if (image != null) {
                image.close();
            }
            return null;
