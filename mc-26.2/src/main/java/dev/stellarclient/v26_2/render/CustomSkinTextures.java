    ...
    private static Identifier ensureRegistered(UUID uuid, String hash, byte[] png) {
        ...
        try {
            ...
            Identifier id = Identifier.fromNamespaceAndPath(
                    StellarClient.MOD_ID, "skins/dynamic/" + uuid.toString().replace("-", ""));
            ...
            FAILED_HASHES.add(hash);
            StellarClient.LOGGER.warn("Failed to upload custom skin for {}: {}", uuid, e.toString());
            if (image != null) {
                image.close();
            }
            return null;
