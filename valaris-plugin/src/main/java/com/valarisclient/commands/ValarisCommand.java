package com.valarisclient.commands;
...
public final class ValarisCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(
        ...
        if (args.length == 0) {
            ...
            if (!player.hasPermission("ValarisClient.use")) {
        ...
        switch (sub) {
            case "reload" -> {
                if (!sender.hasPermission("ValarisClient.reload") && !sender.hasPermission("ValarisClient.admin")) {
                    sender.sendMessage("No permission.");
                    return true;
                }
                plugin.reloadAll();
                sender.sendMessage("ValarisClient reloaded.");
            }
            case "info" -> {
                if (!sender.hasPermission("ValarisClient.info") && !sender.hasPermission("ValarisClient.admin")) {
                    sender.sendMessage("No permission.");
                    return true;
                }
                if (args.length < 2) {
                    sender.sendMessage("Usage: /valaris info <player>");
                    return true;
                }
                ...
                if (sender instanceof Player player) {
                    ...
                    sender.sendMessage("Valaris=" + plugin.detection().isVerified(target.getUniqueId())
            ...
            case "rewards" -> {
                ...
                Text.send(player, "<gray>first_join:</gray> " + (claimed ? "<green>claimed</green>" : "<yellow>available after Valaris handshake</yellow>"));
            }
            case "achievements" -> {
                if (!(sender instanceof Player player)) {
                    return true;
            ...
            default -> sender.sendMessage("Usage: /valaris [reload|info|rewards|achievements]");
        }
        return true;
    }
