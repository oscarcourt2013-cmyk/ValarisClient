...
/**
 * Detects ValarisClient via {@code valarisclient:main} handshake.
...
public final class ClientDetectionService implements PluginMessageListener {
    ...
    public static final String CHANNEL = "valarisclient:main";
    ...
    public ClientDetectionService(ValarisClientPlugin plugin, Database database) {
        this.plugin = plugin;
    ...
    private void handleHandshake(Player player, JsonObject obj) {
        ...
        boolean validClient = "ValarisClient".equalsIgnoreCase(client.trim());
        ...
        if (!validClient || !validProtocol || !validUuid) {
            ...
            reject.addProperty("message", plugin.getConfig().getString(
                    "protocol.reject-message", "Unsupported ValarisClient protocol"));
            sendJson(player, reject);
            plugin.getLogger().warning("Rejected Valaris handshake from " + player.getName()
                    + " (client=" + client + ", protocol=" + protocol + ", uuidMatch=" + validUuid + ")");
            return;
        }

        boolean first = database.findProfile(player.getUniqueId())
                .map(p -> !p.ValarisClient())
                .orElse(true);

        long now = System.currentTimeMillis();
        database.markValarisClient(player.getUniqueId(), player.getName(), version, now);
        ...
        if (plugin.getConfig().getBoolean("verified.grant-permission", true)) {
            player.addAttachment(plugin, "ValarisClient.verified", true);
        }

        JsonObject accept = new JsonObject();
        accept.addProperty("t", "SERVER_ACCEPTED");
        accept.addProperty("message", plugin.getConfig().getString(
                "protocol.accept-message", "Welcome — ValarisClient verified"));
        sendJson(player, accept);

        sendAccountSync(player);

        Bukkit.getScheduler().runTask(plugin, () ->
                Bukkit.getPluginManager().callEvent(new ValarisClientDetectedEvent(player, version, first)));
    }
}
