package com.valerisclient.commands;
...
public final class PrimeCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(
        ...
        if (args.length == 0) {
            ...
            if (!player.hasPermission("ValerisClient.use")) {
        ...
        switch (sub) {
            case "reload" -> {
                if (!sender.hasPermission("ValerisClient.reload") && !sender.hasPermission("ValerisClient.admin")) {
                    sender.sendMessage("No permission.");
                    return true;
                }
                plugin.reloadAll();
                sender.sendMessage("ValerisClient reloaded.");
            }
            case "info" -> {
                if (!sender.hasPermission("ValerisClient.info") && !sender.hasPermission("ValerisClient.admin")) {
                    sender.sendMessage("No permission.");
                    return true;
                }
                if (args.length < 2) {
                ...
                if (sender instanceof Player player) {
                    ...
                    sender.sendMessage("Valeris=" + plugin.detection().isVerified(target.getUniqueId())
            ...
            case "rewards" -> {
                ...
                Text.send(player, "<gray>first_join:</gray> " + (claimed ? "<green>claimed</green>" : "<yellow>available after Valeris handshake</yellow>"));
            }
            case "achievements" -> {
                if (!(sender instanceof Player player)) {
                    return true;
