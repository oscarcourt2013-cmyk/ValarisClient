package com.valerisclient.database;
...
public final class DatabaseFactory {
    ...
    public static Database create(ValerisClientPlugin plugin) {
        FileConfiguration cfg = plugin.getConfig();
        String type = cfg.getString("database.type", "sqlite");
        HikariConfig hikari = new HikariConfig();
        hikari.setPoolName("ValerisClient");
        ...
        if ("mysql".equalsIgnoreCase(type)) {
            ...
            String db = cfg.getString("database.mysql.database", "ValerisClient");
            hikari.setJdbcUrl("jdbc:mysql://" + host + ":" + port + "/" + db
                    + "?useSSL=false&allowPublicKeyRetrieval=true&characterEncoding=utf8");
            hikari.setUsername(cfg.getString("database.mysql.username", "prime"));
            hikari.setPassword(cfg.getString("database.mysql.password", ""));
            hikari.setDriverClassName("com.mysql.cj.jdbc.Driver");
        } else {
            File file = new File(plugin.getDataFolder(), cfg.getString("database.sqlite-file", "ValerisClient.db"));
        ...
        try (Statement st = connection.createStatement()) {
            st.executeUpdate("""
                    CREATE TABLE IF NOT EXISTS players (
                      ...
                      prime_client INTEGER NOT NULL DEFAULT 0,
